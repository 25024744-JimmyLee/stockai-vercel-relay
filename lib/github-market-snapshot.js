...
async function requestGitHub(config, method, path, body = undefined) {
  ...
  console.warn(JSON.stringify({
    event: 'github_market_snapshot_request_failed',
    method,
    path,
    status: response.status,
    githubMessage: typeof payload?.message === 'string' ? payload.message : null,
  }));
...
function githubGitRefPath(config) {
  ...
  return `/repos/${config.repository}/git/refs/heads/${encodedBranch}`;
}
