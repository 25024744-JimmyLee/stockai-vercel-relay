...
function githubGitBlobsPath(config, sha = '') {
  return `/repos/${config.repository}/git/blobs/${encodeURIComponent(sha)}`;
}

function githubGitTreesPath(config, sha = '') {
  return `/repos/${config.repository}/git/trees/${encodeURIComponent(sha)}`;
}

function githubGitCommitsPath(config, sha = '') {
  return `/repos/${config.repository}/git/commits/${encodeURIComponent(sha)}`;
}
